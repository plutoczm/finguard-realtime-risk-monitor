...
def parse_consumer_group_lag(output: str, topic: str) -> int | None:
    ...
    for raw_line in output.splitlines():
        ...
        if len(parts) < 6 or parts[1] != topic or parts[5] == "-":
            continue
        try:
            total += int(parts[5])
            matched = True
...
def wait_for_running_job(
    flink_url: str, job_name: str = DEFAULT_JOB_NAME, timeout_seconds: float = 90.0
) -> dict[str, Any]:
    deadline = time.monotonic() + timeout_seconds
    last_error: Exception | None = None
    while time.monotonic() < deadline:
        try:
            payload = http_json(f"{flink_url.rstrip('/')}/jobs/overview")
            jobs = payload.get("jobs", []) if isinstance(payload, dict) else []
            running = [
                job
                for job in jobs
                if job.get("state") == "RUNNING" and job.get("name") == job_name
            ]
            if running:
...
def job_vertices(flink_url: str, job_id: str) -> list[dict[str, Any]]:
    ...
    vertices = payload.get("vertices", []) if isinstance(payload, dict) else []
    return [item for item in vertices if isinstance(item, dict) and item.get("id")]
...
def aggregated_subtask_metrics(
    ...
    rows = http_json(
        f"{flink_url.rstrip('/')}/jobs/{job_id}/vertices/{vertex_id}/subtasks/metrics?{query}"
    )
    return parse_metric_rows(rows)


def job_metric_values(flink_url: str, job_id: str) -> dict[str, float]:
    query = urllib.parse.urlencode({"get": ",".join(JOB_METRICS)})
    rows = http_json(f"{flink_url.rstrip('/')}/jobs/{job_id}/metrics?{query}")
    return {
        metric_id: values["value"]
        for metric_id, values in parse_metric_rows(rows).items()
        if "value" in values
    }
...
def checkpoint_summary(flink_url: str, job_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "latest_end_to_end_duration_ms": completed.get("end_to_end_duration")
...
def take_sample(
    ...
    return Sample(
        ...
        num_restarts=job_metrics.get("numRestarts"),
    )
...
def summarize_samples(samples: list[Sample], baseline: Sample, events: int) -> dict[str, Any]:
    ...
    if baseline.source_records_out is not None and final.source_records_out is not None:
        ...
        processed_ratio = source_delta / events if events else 1.0
    lags = [sample.lag for sample in samples if sample.lag is not None]
    backpressure = [
        sample.max_backpressure_ms_per_second
        for sample in samples
        if sample.max_backpressure_ms_per_second is not None
    ]
    busy = [
        sample.max_busy_ms_per_second
        for sample in samples
        if sample.max_busy_ms_per_second is not None
    ]
    checkpoints = [
        sample.last_checkpoint_duration_ms
        for sample in samples
        if sample.last_checkpoint_duration_ms is not None
    ]
    restart_delta = None
    ...
    return {
        ...
        "processed_ratio": round(processed_ratio, 4) if processed_ratio is not None else None,
...
def main() -> None:
    ...
    source_vertex = find_source_vertex(vertices)
    baseline = take_sample(
        args.flink_url, job_id, vertices, str(source_vertex["id"]), args.compose,
    ...
    while True:
        ...
        if producer.poll() is not None and producer_finished_at is None:
            producer_finished_at = time.monotonic()
            drain_deadline = producer_finished_at + args.drain_timeout
        if producer_finished_at is not None:
            source_done = (
                baseline.source_records_out is not None
                and sample.source_records_out is not None
                and sample.source_records_out - baseline.source_records_out
                >= args.events * args.min_processed_ratio
            )
            lag_done = sample.lag is not None and sample.lag <= args.max_final_lag
            if source_done and lag_done:
                break
    ...
    producer_elapsed = producer_finished_at - producer_started
    ...
    report = {
        ...
        "environment": {
            ...
            "source_vertex": source_vertex.get("name"),
        },
        "result": {
            "producer_elapsed_seconds": round(producer_elapsed, 3),
            "producer_achieved_qps": round(args.events / producer_elapsed, 3),
            "drain_seconds": round(max(0.0, time.monotonic() - producer_finished_at), 3),
            **result,
    ...
    if checkpoints["completed"] < args.min_completed_checkpoints:
        failures.append(
            f"completed checkpoints {checkpoints['completed']} < {args.min_completed_checkpoints}"
        )
